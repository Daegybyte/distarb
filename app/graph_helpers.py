...
class Graph_Helpers:
    ...
    def graph_it(window, ticker, company, i, date):
        ...
        if date is None:
            ...
            window.graph_widget.addLine(
                x=t, pen="b", name="Transaction Date"
        ...
        dt_list = [
            pendulum.parse(str(dt)).float_timestamp for dt in list(price_history.index)
        ]  # convert pandas datetime to float timestamp
        converted_dates = Graph_Helpers.convert_epoch_to_date(
            dt_list
        )  # convert float timestamp to date string
        zipped = Graph_Helpers.zip_lists_to_dict(
            dt_list, converted_dates
        )  # zip the dates and prices together into a dictionary

        colour = Graph_Helpers.set_colour(i)  # set the colour of the graph line
        print(f"colour: {i}\n type: {type(i)}")

        # format the graph
        plt = window.graph_widget.plot(
            dt_list,
            opens,
            name=company,
            symbolSize=5,
        ...
        try:
            window.graph_widget.getAxis("bottom").setTicks(
                [list(zipped.items())[4 : -1 : len(converted_dates) // 3]]
            )  # start is adjusted to fit the graph window better
        except ValueError:
            window.output_box.appendPlainText(
