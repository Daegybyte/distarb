...
class Helpers:
    ...
    CLEAN_COMPANIES = df_tickers[
        "clean_companies"
    ].values.tolist()  # clean indicates a string that has been stripped of all non-alphanumeric characters and converted to lowercase
    ...
    def get_alignments(cls, search_item, is_business):  # get the alignment of the search_item
        keyboard = clavier.load_qwerty(
            staggering=[0.5, 0.25, 0.5]
        )  # load the qwerty keyboard with standard keyboard staggering
        ...
        if not is_business:
            ...
            for ticker in cls.TICKERS:
                ...
                )  # get the alignment of the search item and the ticker symbol in TICKER_LIST
                key_distance = keyboard.typing_distance(ticker)
                # This includes the initial search item and the ticker symbol for graphing
                if (
                    distance <= 1 and key_distance <= 50
                ):  # Got visually reasonable looking visual results with 50. This would need to change for further exploratory analysis
                    alignment_list.append(
                        (ticker, distance, key_distance)
                    )  # append the ticker and alignment score to the alignment_list

            alignment_list.sort(key=lambda x: (x[1], x[2]), reverse=False)
            distances = (
                ""  # create a single string of the top 10 alignments [0] == ticker, [1] == distance
            )
            tickers = "".join([f"{item[0]}\n" for i, item in enumerate(alignment_list[:10])])[
                :-1
            ]  # create a single string of the top 10 tickers
            ...
            for company in cls.CLEAN_COMPANIES:
                ...
                distance = float(
                    f"{distance:.7f}"
                )  # round the distance to 7 decimal places, seven was chosen based on visual inspection of distances
            ...
            )  # sort the alignment_list by alignment score
            distances = ""
