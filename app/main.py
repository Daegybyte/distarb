...
class UI_MainWindow(object):
    def __init__(self, window):
        ...
        window.setStyleSheet(
            "background-color: grey; border-color: darkgrey; border-style: solid; border-width: 1px; border-radius: 2px;"
        )
        ...
        self.centralwidget.setStyleSheet(
            "background-color: grey; border-color: darkgrey; border-style: solid; border-width: 1px; border-radius: 5px;"
        )
        ...
        self.graph_widget.setMouseEnabled(
            x=False, y=False
        )  # make the graph unadjustable by the user
        ...
        self.btn_search.clicked.connect(
            self.do_it
        )  # connect the search button to the do_it method on click
        ...
        self.ticker_drop_down.currentIndexChanged.connect(
            lambda: self.search_box.setText(self.ticker_drop_down.currentText())
        )  # set the value of the search box to the selected company name
        ...
        self.company_drop_down.currentIndexChanged.connect(
            lambda: self.search_box.setText(self.company_drop_down.currentText())
        )  # set the value of the search box to the selected company name
        ...
        self.output_box.setReadOnly(
            True
        )  # make the output box read only to prevent user input making a mess of the text >:(
    ...
    def do_it(self) -> None:
        ...
        if self.btn_align.isChecked():
            ...
            if text in self.CLEAN_COMPANIES:
                print("\ntext in clean names")
                _, C, is_business = alignment.Helpers.get_alignments(
                    text, True
                )  # get the alignments for the search box text
            else:
                _, T, is_business = alignment.Helpers.get_alignments(
                    text, False
                )  # get the alignments for the search box text
    ...
    def has_errors(self) -> bool:
        try:
            if (
                self.search_box.text() == "" and self.btn_align.isChecked()
            ):  # if the searchbox is empty, create an error popup window
